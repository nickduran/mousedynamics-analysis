...
% TODO: Could update below to save everything using dlmwrite, much easier than fprintf
...
%%% total time across latency, motion, and dwell time %%%%
DV_total_time = t(length(t));

%%%% latency time only %%%%
%%%% motion time only w/ no latency and dwell time (NEW oct28 NOW DOES NOT INCLUDE DWELL TIME) %%%%
%%%% dwell time only %%%%
...
DV_inmot = (t(length(t))-DV_latency)-DV_dwell; % motion time (ms) - time spent moving NEW oct28 OUTSIDE ESCAPE AND DWELL REGIONS

%%%% distance (euclidean) entire length %%%%
distx = (x(2:length(x))-x(1:length(x)-1)).^2; % (x2-x1)^2
disty = (y(2:length(y))-y(1:length(y)-1)).^2; % (y2-y1)^2
DV_dist = sum(sqrt(distx+disty)); % distance traveled by one trajectory, includes the entire length of the trajectory (latency and dwell)

%%%% dist w/ no latency NEW oct28 AND DWELL %%%%
distlatx = (xtrim(2:length(xtrim))-xtrim(1:length(xtrim)-1)).^2; % (x2-x1)^2 % includes dwell region
distlaty = (ytrim(2:length(ytrim))-ytrim(1:length(ytrim)-1)).^2; % (y2-y1)^2 % includes dwell region
distdwlx = (xtrim3(2:length(xtrim3))-xtrim3(1:length(xtrim3)-1)).^2; % (x2-x1)^2 NEW oct28 dwell region travel
distdwly = (ytrim3(2:length(ytrim3))-ytrim3(1:length(ytrim3)-1)).^2; % (y2-y1)^2 NEW oct28 dwell region travel

dist_outside = sum(sqrt(distlatx+distlaty)); % distance traveled by one trajectory outside of the latency ESCAPE pixel region
dist_dwell = sum(sqrt(distdwlx+distdwly)); % distance traveled by one trajectory inside DWELL pixel region
DV_dist_inmot = dist_outside - dist_dwell;
...
xlatency = x(1:latency_indx-1); % the truncated x trajectory only in latency region (e.g., 100 pixel radius), for computing x-flips in latency
ylatency = y(1:latency_indx-1); % the truncated y trajectoy only in latency region (e.g., 100 pixel radius), for computing y-flips in latency (not done here)
...
DV_xflp_mot = xflip(xtrim)-DV_xflp_dwl; % NEW oct28 flips that do not include dwell 
DV_aflp_mot = (xflip(trajAng(latency_indx:length(trajAng))))-DV_aflp_dwl; % NEW oct28 flips that do not include dwell 

%%% entropy along x-axis %%% 
%%% entropy along y-axis %%%
%%% entropy along trajectory angle %%%
DV_xe = SampEn( 5, .2, x, 1 );
DV_ye = SampEn( 5, .2, y, 1 );
% below now working, problems with sampen, need to download and install new version
